...
print(f"Average GPU Time: {time_gpu:.4f} seconds")
print(f"Average ndimage Time: {time_nd:.4f} seconds")
...
axs[1].set_title(f"GPU Scaled Image\nTime: {time_gpu:.4f} s")
axs[1].axis("on")
axs[1].grid(True)

axs[2].imshow(scaled_image_nd, cmap="gray", interpolation="nearest")
axs[2].set_title(f"ndimage Scaled Image\nTime: {time_nd:.4f} s")
axs[2].axis("on")
axs[2].grid(True)
